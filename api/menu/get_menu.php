<?php
/**
 * Get Menu Items API Endpoint
 * Returns menu items with categories, drink options, extras, and legends
 */
...
try {
    // Get currency from query parameter (default: PHP)
    ...
    // Get menu items with their details
    ...
    while ($menu = $menuResult->fetch_assoc()) {
        ...
        if (isset($categories[$category_id])) {
            $categories[$category_id]['items'][] = $menuItem;
        }
    }
    
    // Convert to array format
    $menuData = array_values($categories);
    ...
    sendErrorResponse('An error occurred while fetching menu', 500);
} finally {
