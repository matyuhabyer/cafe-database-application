...
const TAB_TRACKING_KEY = 'wr_active_tabs';
const TAB_SESSION_KEY = 'wr_tab_id';
...
(function initializeSessionReset() {
    try {
        const raw = localStorage.getItem(TAB_TRACKING_KEY);
        let tabList = [];
        if (raw) {
            try {
                ...
                console.warn('Unable to parse active tabs list, resetting.', error);
            }
        }
        ...
        if (!tabSet.has(tabId)) {
            tabSet.add(tabId);
            localStorage.setItem(TAB_TRACKING_KEY, JSON.stringify([...tabSet]));
        }

        window.addEventListener('beforeunload', () => {
            try {
                const latestRaw = localStorage.getItem(TAB_TRACKING_KEY);
                const latestList = latestRaw ? JSON.parse(latestRaw) : [];
                const filtered = Array.isArray(latestList)
                    ? latestList.filter(id => id !== tabId)
                    : [];
                localStorage.setItem(TAB_TRACKING_KEY, JSON.stringify(filtered));
            } catch (error) {
                console.warn('Error updating tab tracking on unload:', error);
            }
        });
    } catch (error) {
        console.warn('Session reset initialization failed:', error);
    }
})();
