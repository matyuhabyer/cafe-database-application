...
function getOrderItemImageMarkup(item) {
    const resolvedSrc = resolveImageUrl(item.image_url);
    if (resolvedSrc) {
        const alt = escapeAttribute(item.name || 'Order item');
        return `<img src="${resolvedSrc}" alt="${alt}" loading="lazy" decoding="async">`;
    }
    return '<span class="order-item-placeholder">🍽️</span>';
}
...
function renderOrderDetails(order) {
    ...
    const fallbackPoints = order.loyalty && order.loyalty.card_number
        ? Math.max(0, Math.floor(Number(order.total_amount || 0) / 50))
        : 0;
    const displayPoints = numericPoints > 0 ? numericPoints : (fallbackPoints > 0 ? fallbackPoints : null);
    ...
    let html = `
        <div class="order-details-card">
            ...
            ${renderPrintReceiptButton(order)}
...
function renderOrderItems(items) {
    ...
    return items.map(item => {
        ...
        const imageMarkup = getOrderItemImageMarkup(item);
...
function renderPrintReceiptButton(order) {
    // Show print receipt button if there's a transaction
    if (!order.transaction || !order.transaction.transaction_id) {
        return '';
    }
    
    const transactionId = order.transaction.transaction_id;
    const orderId = order.order_id;
    
    return `
        <div class="action-buttons" style="margin-top: 20px;">
            <button class="btn-action btn-print-receipt" onclick="printReceipt(${transactionId}, ${orderId})">
                🖨️ Print Receipt
            </button>
        </div>
    `;
...
function printReceipt(transactionId, orderId) {
    // Redirect to receipt page with transaction_id
    if (transactionId) {
        window.location.href = `receipt.html?transaction_id=${transactionId}`;
    } else if (orderId) {
        // Fallback to order_id if transaction_id is not available
