...
CREATE TABLE IF NOT EXISTS `cafe_db`.`OrderTbl` (
  ...
  `status` ENUM('pending', 'in_preparation', 'ready_for_pickup', 'completed', 'cancelled') NOT NULL DEFAULT 'pending',
  `branch_id` INT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `cafe_db`.`OrderHistory` (
  ...
  `employee_id` INT NULL DEFAULT NULL,
  `status` ENUM('pending', 'in_preparation', 'ready_for_pickup', 'completed', 'cancelled') NOT NULL,
